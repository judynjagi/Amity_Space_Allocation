...
class TestCreateRoom(unittest.TestCase):
	...
	def test_allocate_person_exists(self):
		self.assertEqual(self.space.allocatestaff("RYAN CATEY", "N"),"The name does not exist, please enter a valid name")
		self.assertEqual(self.space.allocatefellow("MARGARET AISHA", "N"),"The name does not exist, please enter a valid name")
	
	def test_allocate_staff_to_livingroom(self):
		self.space.create_room("Room4", "LIVINGSPACE")
		self.space.add_person("JOHN", "NGATIA", "STAFF", "Y")
		allocate = self.space.allocatestaff("JOHN NGATIA", "Y")
		self.assertEqual(allocate, "staff cannot be allocated a livingspace")

	def test_allocate_staff_to_office(self):
		self.assertTrue(self.staffs)

	def test_allocate_fellow_to_office(self):
		self.assertTrue(self.fellows)
