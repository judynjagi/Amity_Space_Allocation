...
class TestCreateRoom(unittest.TestCase):
	...
	def test_load_people(self):
		...
		path = os.path.realpath('test_files/empty.txt')
		results = self.space.load_people(path)
		# path = os.path.dirname(os.path.realpath(__file__))+"/" + results
		self.assertEqual(results, 'Empty file!.')
		

	def test_load_people_file_exists(self):
		result = self.space.load_people("sample.txt")
		path = os.path.dirname(os.path.realpath(__file__))+"/" + result
		self.assertEqual(result, "Invalid path.")
		self.assertFalse(os.path.isfile("sample.txt"))
		self.assertTrue(path)
		# test for another file extension other than .txt
		result = self.space.load_people("people.py")
		self.assertEqual(result, "Invalid path.")
	...
	def test_print_allocations(self):
		...
		self.assertEqual(len(self.space.allocated_rooms), 4)

	def test_print_unallocated(self):
		file = self.space.print_unallocated("unallocated.txt")
		self.assertFalse(os.path.exists('unallocated.txt'))
